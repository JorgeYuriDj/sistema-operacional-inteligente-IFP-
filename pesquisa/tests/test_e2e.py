...
def test_browser_desktop_mobile_security_persistence_and_load(tmp_path, engine):
    ...
    try:
        with sync_playwright() as p:
            ...
            expect(page.get_by_role("heading",name="Da faculdade para a profissão.")).to_be_visible()
            expect(page.locator('[data-step]:visible')).to_have_count(1)
            assert page.evaluate("document.documentElement.scrollWidth <= innerWidth")
            page.screenshot(path=str(screenshots/"public-desktop.png"),full_page=True)
            page.locator('#next').click()
            expect(page.locator('#step-label')).to_have_text("Pergunta 1 de 5")
            page.locator('[name=stage][value=s12]').check()
            page.locator('#next').click()
            page.locator('[name=area][value=personal]').check()
            page.locator('#next').click()
            page.locator('#next').click()
            expect(page.locator('#form-error')).to_contain_text("selecione uma ou duas")
            page.locator('#previous').click()
            expect(page.locator('[name=area][value=personal]')).to_be_checked()
            page.locator('#previous').click()
            expect(page.locator('[name=stage][value=s12]')).to_be_checked()
            ...
            page.locator('#next').click()
            expect(page.locator('#step-label')).to_have_text('Última etapa')
            ...
            expect(mp.get_by_role('heading',name='Da faculdade para a profissão.')).to_be_visible()
            assert mp.evaluate("document.documentElement.scrollWidth <= innerWidth")
            assert mp.locator('.question legend').first.bounding_box()['y'] < 620
            assert mp.locator('.option').first.evaluate("el => parseFloat(getComputedStyle(el).fontSize)") >= 16
            assert mp.locator('.option').first.bounding_box()['height'] >= 44
            expect(mp.locator('[data-step]:visible')).to_have_count(1)
            mp.screenshot(path=str(screenshots/"public-mobile.png"),full_page=True)
            mp.locator('[name=stage][value=s34]').check();mp.locator('#next').click()
            mp.locator('[name=area][value=group]').check();mp.locator('#next').click()
            mp.locator('[name=gaps][value=classes]').check();mp.locator('#next').click()
            mp.locator('[name=format][value=workshops]').check();mp.locator('#next').click()
            mp.locator('[name=wish]').fill("Ter mais oportunidades de prática supervisionada.")
            mp.locator('#next').click()
            assert mp.locator('[name=email]').evaluate("el => parseFloat(getComputedStyle(el).fontSize)") >= 16
